...
# Load data
setwd("/home/data/Harvard/king/ls3/HL/tables/models/")
mydata <- read.csv('/home/data/Harvard/king/ls3/HL/data/total_sample_n4790.csv')

x <- mydata[,4:13] # These should be the 10 variables in your final model

Y <- ifelse(mydata$homeless_12m == 1, 1, 0)
...
set.seed(20230503)
ranger_sl <- SuperLearner(Y = Y, X = x, family = binomial(),
                          obsWeights = weight,
...
out_cv_pred <- data.frame(masterid = mydata$masterid,
                          ...
                          fold = fold,
                          cv_pred_prob)
write.csv(out_cv_pred,'ranger_cv_pred_prob.csv',row.names=F)
